          ...
          <div className="flex items-center gap-2">
            ...
            {user ? (
              <div className="hidden lg:flex items-center gap-2">
                {isAdmin && (
                  <Link to="/admin" className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-gray-400 hover:text-blue-400 hover:bg-blue-500/10 transition-all text-sm font-medium">
                    <ShieldCheck className="w-4 h-4" />
                    <span>{lang === 'ar' ? 'لوحة التحكم' : 'Admin'}</span>
                  </Link>
                )}
                <button onClick={logout}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-all text-sm font-medium">
                  <LogOut className="w-4 h-4" />
                  <span className="hidden xl:inline">{lang === 'ar' ? 'خروج' : 'Logout'}</span>
                </button>
              </div>
            ) : (
              <Link to="/login"
                className="hidden lg:flex items-center gap-1.5 px-4 py-2 rounded-xl bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-all text-sm font-medium">
